...
int main(int argc, char** argv){
	...
	for(real zeta = zeta_min; zeta<=zeta_max; zeta+=zeta_step){
		int r1up=0,r2up=0;
		N = 1<<Npower;
		...
		cout << lambda << " : ";
		cout.flush();
		auto r1r2hist = c2.generate_history_changing_lambda(lambda_init,lambda_final,lambda_step,t_final);
		hsize_t dim[] = {r1r2hist.size()};   /* Dataspace dimensions */
	    H5::DataSpace dataspace( 1, dim );
	    std::stringstream runname;
	    runname << "run_N" <<N << "_zeta"<<lambda;
